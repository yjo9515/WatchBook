  ...
  startLogin() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsondata['result'] == false) {
        ...
        } else if (jsondata["message"] == "비밀번호가 틀립니다. 다시 확인해 주십시오.") {
          setState(() {
            error = true;
            errormsg = "비밀번호가 틀립니다.";
          });
        } else if (jsondata["message"] == "등록되지 않은 아이디입니다.") {
          setState(() {
            error = true;
            errormsg = "등록되지 않은 아이디입니다.";
          });
        } else if (jsondata["message"] == "탈퇴한 계정입니다.") {
          setState(() {
            error = true;
            errormsg = "탈퇴한 계정입니다.";
          });
        } else if (jsondata["message"] == "회원정보를 업데이트하는데 실패하였습니다.") {
          setState(() {
            error = true;
            errormsg = "정보 업데이트를 실패하였습니다.";
          });
        } else {
          setState(() {
            error = true;
        ...
        String _tokenValue = jsondata["token"];
        String _idValue = _id.text;
        ...
        if (_tokenValue != null) {
          ...
          sharedPreferences.setString('token', _tokenValue);
          sharedPreferences.setString('id', _idValue);
